...
export default function NowPlaying({ roomCode }) {
    ...
    const progressRef = useRef(0);                  // local ticker reference
    const isPlayingRef = useRef(false);

    // Fetch now-playing and reset local state
    const fetchNowPlaying = useCallback(async () => {
        try {
            ...
            // silently ignore – player stays in last known state
        } finally {
            setLoading(false);
        }
    }, [roomCode]);
    ...
    useEffect(() => {
        const tick = setInterval(() => {
            ...
            progressRef.current = Math.min(
                ...
                track?.duration ?? progressRef.current
            );
        ...
        >
            ...
            <div style={{
                        ...
                        <span
                            ...
                            style={{
                                ...
                                animation: track?.isPlaying
                                    ? `eq-bar-${i} 0.${5 + i}s ease-in-out infinite alternate`
                                    : 'none',
                ...
                <span style={{
                    ...
                    {loading ? 'Loading...' : track?.isPlaying ? 'Now Playing' : track ? 'Paused' : 'Nothing Playing'}
                </span>
            </div>
